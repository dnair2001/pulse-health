...
| `npm test` | Full suite: 170 pytest + 106 Karma = 276, plus React's Vitest suite |
| `npm run lint` | ruff + Angular eslint (includes a complexity budget — see AGENTS.md) + React oxlint |
...
| `npm run check:duplication` | jscpd duplicate-code budget over mock-api + portal-angular |
...
3. `apps/portal-angular` is deliberately legacy and is the reference implementation for the
   in-progress React port in `apps/portal-react`. Do not modernize it, and do not delete it.
